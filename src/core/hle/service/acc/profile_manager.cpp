...
ProfileManager::ProfileManager() {
    ...
    if (user_count == 0) {
        CreateNewUser(UUID::MakeRandom(), "uzuy");
        WriteUserSaveFile();
    }
...
void ProfileManager::ParseUserSaveFile() {
    const auto save_path(FS::GetUzuyPath(FS::UzuyPath::NANDDir) / ACC_SAVE_AVATORS_BASE_PATH /
                         "profiles.dat");
    const FS::IOFile save(save_path, FS::FileAccessMode::Read, FS::FileType::BinaryFile);

    if (!save.IsOpen()) {
        LOG_WARNING(Service_ACC, "Failed to load profile data from save data... Generating new "
                                 "user 'uzuy' with random UUID.");
        return;
    }
...
void ProfileManager::WriteUserSaveFile() {
    ...
    const auto raw_path(FS::GetUzuyPath(FS::UzuyPath::NANDDir) / "system/save/8000000000000010");
