...
std::optional<NcaID> RegisteredCache::GetNcaIDFromMetadata(u64 title_id,
    ...
    const auto res1 = CheckMapForContentRecord(uzuy_meta, title_id, type);
    if (res1)
        return res1;
...
void RegisteredCache::AccumulateUzuyMeta() {
    const auto meta_dir = dir->GetSubdirectory("uzuy_meta");
    if (meta_dir == nullptr) {
        return;
    }
    ...
    for (const auto& file : meta_dir->GetFiles()) {
        ...
        CNMT cnmt(file);
        uzuy_meta.insert_or_assign(cnmt.GetTitleID(), std::move(cnmt));
    }
}
...
std::optional<u32> RegisteredCache::GetEntryVersion(u64 title_id) const {
    ...
    const auto uzuy_meta_iter = uzuy_meta.find(title_id);
    if (uzuy_meta_iter != uzuy_meta.cend()) {
        return uzuy_meta_iter->second.GetTitleVersion();
    }
...
InstallResult RegisteredCache::InstallEntry(const NCA& nca, TitleType type,
    ...
    if (!RawInstallUzuyMeta(new_cnmt)) {
        return InstallResult::ErrorMetaFailed;
    }
...
bool RegisteredCache::RemoveExistingEntry(u64 title_id) const {
    ...
    // If patch entries for any program exist in uzuy meta, remove them
    for (u8 i = 0; i < 0x10; i++) {
        const auto meta_dir = dir->CreateDirectoryRelative("uzuy_meta");
...
bool RegisteredCache::RawInstallUzuyMeta(const CNMT& cnmt) {
    ...
    Refresh();
    return std::find_if(uzuy_meta.begin(), uzuy_meta.end(),
