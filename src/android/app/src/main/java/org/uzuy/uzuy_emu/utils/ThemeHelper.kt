...
import org.uzuy.uzuy_emu.ui.main.ThemeProvider
...
object ThemeHelper {
    ...
    fun setTheme(activity: AppCompatActivity) {
        ...
        when (Theme.from(IntSetting.THEME.getInt())) {
            Theme.Default -> activity.setTheme(R.style.Theme_Uzuy_Main)
            Theme.MaterialYou -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                    activity.setTheme(R.style.Theme_Uzuy_Main_MaterialYou)
                } else {
                    activity.setTheme(R.style.Theme_Uzuy_Main)
                }
            }
        }
        ...
        if (BooleanSetting.BLACK_BACKGROUNDS.getBoolean() && isNightMode(activity)) {
            activity.setTheme(R.style.ThemeOverlay_Uzuy_Dark)
        }
    }
