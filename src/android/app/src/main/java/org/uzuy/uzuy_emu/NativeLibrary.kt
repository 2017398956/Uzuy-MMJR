...
import org.uzuy.uzuy_emu.fragments.CoreErrorDialogFragment
import org.uzuy.uzuy_emu.utils.DocumentsTree
 ...
 * with the native side of the Uzuy code.
 */
object NativeLibrary {
    ...
    init {
        try {
            System.loadLibrary("uzuy-android")
    ...
    fun openContentUri(path: String?, openmode: String?): Int {
        return if (DocumentsTree.isNativePath(path!!)) {
            UzuyApplication.documentsTree!!.openContentUri(path, openmode)
        } else {
    ...
    fun getSize(path: String?): Long {
        return if (DocumentsTree.isNativePath(path!!)) {
            UzuyApplication.documentsTree!!.getFileSize(path)
        } else {
    ...
    fun exists(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            UzuyApplication.documentsTree!!.exists(path)
        } else {
    ...
    fun isDirectory(path: String?): Boolean {
        return if (DocumentsTree.isNativePath(path!!)) {
            UzuyApplication.documentsTree!!.isDirectory(path)
        } else {
    ...
    fun getParentDirectory(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            UzuyApplication.documentsTree!!.getParentDirectory(path)
        } else {
            path
        }
    ...
    fun getFilename(path: String): String =
        if (DocumentsTree.isNativePath(path)) {
            UzuyApplication.documentsTree!!.getFilename(path)
        } else {
    ...
    /**
     * Logs the Uzuy version, Android version and, CPU.
     */
