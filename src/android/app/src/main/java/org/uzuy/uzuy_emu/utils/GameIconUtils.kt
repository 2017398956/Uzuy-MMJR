...
object GameIconUtils {
    private val imageLoader = ImageLoader.Builder(UzuyApplication.appContext)
        .components {
            add(GameIconKeyer())
            add(GameIconFetcher.Factory())
        }
        .memoryCache {
            MemoryCache.Builder(UzuyApplication.appContext)
                .maxSizePercent(0.25)
                .build()
        }
        .build()
    ...
    fun loadGameIcon(game: Game, imageView: ImageView) {
        val request = ImageRequest.Builder(UzuyApplication.appContext)
            .data(game)
            .target(imageView)
    ...
    suspend fun getShortcutIcon(lifecycleOwner: LifecycleOwner, game: Game): IconCompat {
        val layerDrawable = ResourcesCompat.getDrawable(
            UzuyApplication.appContext.resources,
            R.drawable.shortcut,
            null
        ) as LayerDrawable
        layerDrawable.setDrawableByLayerId(
            R.id.shortcut_foreground,
            getGameIcon(lifecycleOwner, game).toDrawable(UzuyApplication.appContext.resources)
        )
        val inset = UzuyApplication.appContext.resources
