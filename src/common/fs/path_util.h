...
enum class UzuyPath {
    UzuyDir,        // Where uzuy stores its data.
    ...
    ScreenshotsDir, // Where uzuy screenshots are stored.
...
/**
 * Gets the filesystem path associated with the UzuyPath enum.
 *
 * @param uzuy_path UzuyPath enum
 *
 * @returns The filesystem path associated with the UzuyPath enum.
 */
[[nodiscard]] const std::filesystem::path& GetUzuyPath(UzuyPath uzuy_path);

/**
 * Gets the filesystem path associated with the UzuyPath enum as a UTF-8 encoded std::string.
 *
 * @param uzuy_path UzuyPath enum
 *
 * @returns The filesystem path associated with the UzuyPath enum as a UTF-8 encoded std::string.
 */
[[nodiscard]] std::string GetUzuyPathString(UzuyPath uzuy_path);

/**
 * Sets a new filesystem path associated with the UzuyPath enum.
 * If the filesystem object at new_path is not a directory, this function will not do anything.
 *
 * @param uzuy_path UzuyPath enum
 * @param new_path New filesystem path
 */
void SetUzuyPath(UzuyPath uzuy_path, const std::filesystem::path& new_path);

#ifdef _WIN32
template <typename Path>
void SetUzuyPath(UzuyPath uzuy_path, const Path& new_path) {
    if constexpr (IsChar<typename Path::value_type>) {
        SetUzuyPath(uzuy_path, ToU8String(new_path));
    } else {
        SetUzuyPath(uzuy_path, std::filesystem::path{new_path});
    }
}
#endif
...
/**
 * Gets the relevant paths for uzuy to store its data based on the given XDG environment variable.
 ...
 *
 * @returns The path where uzuy should store its data.
 */
...
// i.e. "C:\Users\Uzuy\Documents\save.bin" becomes {"C:", "Users", "Uzuy", "Documents", "save.bin" }
