...
static void PrintHelp(const char* argv0) {
    LOG_INFO(Network,
             ...
             "--web-api-url       uzuy Web API url\n"
             "--ban-list-file     The file for storing the room ban list\n"
             "--log-file          The file for storing the room log\n"
             "--enable-uzuy-mods Allow uzuy Community Moderators to moderate on your room\n"
...
static void PrintVersion() {
    LOG_INFO(Network, "uzuy dedicated room {} {} Libnetwork: {}", Common::g_scm_branch,
             Common::g_scm_desc, Network::network_version);
}

/// The magic text at the beginning of a uzuy-room ban list file.
static constexpr char BanListMagic[] = "UzuyRoom-BanList-1";
...
int main(int argc, char** argv) {
    ...
    std::string log_file = "uzuy-room.log";
    std::string bind_address;
    u64 preferred_game_id = 0;
    u32 port = Network::DefaultRoomPort;
    u32 max_members = 16;
    bool enable_uzuy_mods = false;
    ...
    static struct option long_options[] = {
        ...
        {"enable-uzuy-mods", no_argument, 0, 'e'},
    ...
    if (announce) {
        if (username.empty()) {
            ...
            PadToken(token);
            Settings::values.uzuy_username = UsernameFromDisplayToken(token);
            username = Settings::values.uzuy_username.GetValue();
            Settings::values.uzuy_token = TokenFromDisplayToken(token);
        } else {
            LOG_INFO(Network, "Hosting a public room");
            Settings::values.web_api_url = web_api_url;
            Settings::values.uzuy_username = username;
            Settings::values.uzuy_token = token;
        }
    }
    if (!announce && enable_uzuy_mods) {
        enable_uzuy_mods = false;
        LOG_INFO(Network, "Can not enable uzuy Moderators for private rooms");
    }
...
#else
        LOG_INFO(Network,
                 "uzuy Web Services is not available with this build: validation is disabled.");
    ...
    if (auto room = network.GetRoom().lock()) {
        ...
        if (!room->Create(room_name, room_description, bind_address, static_cast<u16>(port),
                          ...
                          std::move(verify_backend), ban_list, enable_uzuy_mods)) {
